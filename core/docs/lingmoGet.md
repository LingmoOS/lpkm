...
## <a name="Option Syntax"/>Option Syntax
  ...
  You may freely mix different option styles. Thus you may write:
      ...
      lingmoGet -r --tries=10 https://example.com/ -o log
  ...
  also clear the lists in `.wget2rc`.
      ...
      lingmoGet --no-exclude-directories -X /priv,/trash
...
### `--stats-site=[FORMAT:]FILE`
  ...
  ID,ParentID,URL,Status,Link,Method,Size,SizeDecompressed,TransferTime,ResponseTime,Encoding,Verification
    ...
    `Encoding` 0,1,2,3,4,5 mean server side compression was 'identity', 'lingmoUnPkg', 'deflate', 'lzma/xz', 'bzip2', 'brotli', 'zstd', 'lzip'
...
### `--bind-interface=INTERFACE`
  ...
  However, the option works only when lingmoGet is run with elevated privileges
  (On GNU/Linux: root / sudo or `sudo setcap cap_net_raw+ep <path to wget|lingmoGet>`).
...
### `--retry-on-http-error=list`
  ...
  something with exceptions. For example, retry every failed download if error code is not 404:
      ...
      lingmoGet --retry-on-http-error=*,\!404 https://example.com/
...
### `-c`, `--continue`
  ...
  previous instance of Wget2, or by another program.  For instance:
      ...
      lingmoGet -c https://example.com/tarball.gz
  ...
  `lingmoGet -c` to download just the new portion that's been appended to a data collection or log file.
...
### `--spider`
  ...
  just check that they are there.  For example, you can use Wget2 to check your bookmarks:
      ...
      lingmoGet --spider --force-html -i bookmarks.html
...
### `-Q quota`, `--quota=quota`
  ...
  Note that quota will never affect downloading a single file.  So if you specify
      ...
      lingmoGet -Q10k https://example.com/bigfile.gz
...
### `-x`, `--force-directories`
  ...
  `lingmoGet -x https://example.com/robots.txt` will save the downloaded file to `example.com/robots.txt`.
...
### `--protocol-directories`
  ...
  Use the protocol name as a directory component of local file names.  For example, with this option, `lingmoGet -r
...
### `--load-cookies file`
  ...
  support:
      ...
      lingmoGet --no-cookies --header "Cookie: <name>=<value>"
...
### `--header=header-line`
  ...
  You may define more than one additional header by specifying `--header` more than once.
      ...
      lingmoGet --header='Accept-Charset: iso-8859-2' \
  ...
  instructs Wget2 to connect to localhost, but to specify `example.com` in the "Host" header:
      ...
      lingmoGet --header="Host: example.com" http://localhost/
...
### `--post-data=string`, `--post-file=file`
  ...
  Please note that lingmoGet does not require the content to be of the form "key1=value1&key2=value2", and neither does
  ...
  presumably only accessible to authorized users:
      ...
      lingmoGet --save-cookies cookies.txt \
           --post-data  'user=foo&password=bar' \
           http://example.com/auth.php

      # Now grab the page or pages we care about.
      lingmoGet --load-cookies cookies.txt \
...
### `--body-data=Data-String`, `--body-file=Data-File`
  ...
  changed so that lingmoGet sends the `--body-file` as a complete file instead of sending its contents to the server.
...
### `--content-on-error`
  ...
  If this is set to on, lingmoGet will not skip the content when the server responds with a http status code that
  indicates error.
...
### `--compression=TYPE`
  ...
  If this TYPE(`identity`, `lingmoUnPkg`, `deflate`, `xz`, `lzma`, `br`, `bzip2`, `zstd`, `lzip` or any combination of it)
...
### `--delete-after`
  ...
  fetching popular pages through a proxy, e.g.:
      ...
      lingmoGet -r -nd --delete-after https://example.com/~popular/page/
...
### `-p`, `--page-requisites`
  ...
  If one executes the command:
      ...
      lingmoGet -r -l 2 https://<site>/1.html
  ...
  determine where to stop the recursion.  However, with this command:
      ...
      lingmoGet -r -l 2 -p https://<site>/1.html

  all the above files and 3.html's requisite 3.gif will be downloaded.  Similarly,

      lingmoGet -r -l 1 -p https://<site>/1.html

  will cause 1.html, 1.gif, 2.html, and 2.gif to be downloaded.  One might think that:

      lingmoGet -r -l 0 -p https://<site>/1.html
  ...
  locally, this author likes to use a few options in addition to `-p`:
      ...
      lingmoGet -E -H -k -K -p https://<site>/<document>
...
### `--regex-type=regextype`
  ...
  lingmoGet has to be compiled with libpcre support.
...
### `--ignore-tags=list`
  ...
  like:
      ...
      lingmoGet --ignore-tags=a,area -H -k -K -r https://<site>/<document>
...
### `-I list`, `--include-directories=list`
  ...
  wildcards.
      ...
      lingmoGet -r https://webpage.domain --include-directories=*/pub/*/
...
### `-X list`, `--exclude-directories=list`
  ...
  wildcards.
      ...
      lingmoGet -r https://gnu.org --exclude-directories=/software
...
### `--filter-mime-type=list`
  ...
  'application/octet-stream'. For example, download everything except images:
      ...
      lingmoGet -r https://<site>/<document> --filter-mime-type=*,\!image/*
  ...
  download every file that is compatible with LibreOffice Writer from a website using the recursive mode:
      ...
      lingmoGet -r https://<site>/<document> --filter-mime-type=$(sed -r '/^MimeType=/!d;s/^MimeType=//;s/;/,/g' /usr/share/applications/libreoffice-writer.desktop)
...
# <a name="Bugs"/>Bugs
  ...
  You are welcome to submit bug reports via the [GNU Wget2 bug tracker](https://gitlab.com/gnuwget/lingmoGet/issues).
  ...
  2.  Try to repeat the bug in as simple circumstances as possible.  E.g. if Wget2 crashes while downloading `lingmoGet -rl0
