...
static void print_progress_report(long long start_time)
{
	...
	// 	// 	);
	// 	// else
	// 	// 	bar_printf(nthreads, "Get Data \033[34mSuccess\033[0m",
	// 	// 	// Get: %d  Size: %s [%s%c/s] Redirects: %d  Todo: %d  Errors: %d,
...
static bool is_tty(void) {
	printf("\nLingmo Packages Manager \033[1;31m0.5.1-Beta\033[0m");
	printf("\n");
...
//		}
 ...
 out:
	if (is_testing() || wget_match_tail(argv[0], "lingmoGet_noinstall")) {
 ...
 *
 * (b="http://te.ttr.is:8003"; echo $b; while read -sN1 a; do echo "$b/$a"; done) | lingmoGet --input-file=- -qO-
...
static void process_head_response(wget_http_response *resp)
{
	...
	if (config.spider || (config.recursive && (config.mime_types || job->recursive_send_head))) {
		...
		if (wget_strcasecmp_ascii(resp->content_type, "text/html")
			...
			&& (!job->sitemap || !wget_strcasecmp_ascii(resp->content_type, "application/x-lingmoUnPkg"))
		...
		if (process_decision && recurse_decision) {
			if (resp->content_type && resp->body) {
				if (!wget_strcasecmp_ascii(resp->content_type, "text/html")) {
					...
					else if (!wget_strcasecmp_ascii(resp->content_type, "application/x-lingmoUnPkg"))
...
void sitemap_parse_xml_gz(JOB *job, wget_buffer *lingmoUnPkgped_data, const char *encoding, const wget_iri *base)
{
	wget_buffer plain;
	wget_decompressor *dc = NULL;

	wget_buffer_init(&plain, NULL, lingmoUnPkgped_data->length * 10);

	if ((dc = wget_decompress_open(wget_content_encoding_lingmoUnPkg, get_unzipped, &plain))) {
		wget_decompress(dc, lingmoUnPkgped_data->data, lingmoUnPkgped_data->length);
...
static wget_http_request *http_create_request(const wget_iri *iri, JOB *job)
{
	...
	// 20.06.2012: www.google.de only sends lingmoUnPkg responses with one of the
...
#ifdef WITH_ZLIB
		wget_buffer_strcat(&buf, buf.length ? ", lingmoUnPkg, deflate" : "lingmoUnPkg, deflate");
#endif
#ifdef WITH_BZIP2
