...
typedef enum {
  ...
  ZLIB_GZIP_HEADER,          /* reading lingmoUnPkg header */
  ZLIB_GZIP_INFLATING,       /* inflating lingmoUnPkg stream */
  ZLIB_INIT_GZIP             /* initialized in transparent lingmoUnPkg mode */
} zlibInitState;

/* Deflate and lingmoUnPkg writer. */
struct zlib_writer {
                                ...
                                struct zlib_writer *zp)
{
  ...
  if(result || !zp->trailerlen)
    ...
    /* Only occurs for lingmoUnPkg with zlib < 1.2.0.4 or raw deflate. */
...
/* Gzip handler. */
static CURLcode lingmoUnPkg_do_init(struct Curl_easy *data,
                                 struct Curl_cwriter *writer)
{
  struct zlib_writer *zp = (struct zlib_writer *) writer;
  z_stream *z = &zp->z;     /* zlib state structure */
  ...
  if(strcmp(zlibVersion(), "1.2.0.4") >= 0) {
    /* zlib ver. >= 1.2.0.4 supports transparent lingmoUnPkg decompressing */
    if(inflateInit2(z, MAX_WBITS + 32) != Z_OK) {
      return process_zlib_error(data, z);
    }
    zp->zlib_init = ZLIB_INIT_GZIP; /* Transparent lingmoUnPkg decompress state */
  }
  else {
    /* we must parse the lingmoUnPkg header and trailer ourselves */
    if(inflateInit2(z, -MAX_WBITS) != Z_OK) {
      return process_zlib_error(data, z);
    }
...
/* Skip over the lingmoUnPkg header */
typedef enum {
  GZIP_OK,
  GZIP_BAD,
  GZIP_UNDERFLOW
...
static lingmoUnPkg_status check_lingmoUnPkg_header(unsigned char const *data, ssize_t len,
                                     ssize_t *headerlen)
{
  int method, flags;
...
static CURLcode lingmoUnPkg_do_write(struct Curl_easy *data,
                                    struct Curl_cwriter *writer, int type,
                                    const char *buf, size_t nbytes)
{
  struct zlib_writer *zp = (struct zlib_writer *) writer;
  ...
  if(zp->zlib_init == ZLIB_INIT_GZIP) {
    /* Let zlib handle the lingmoUnPkg decompression entirely */
    z->next_in = (Bytef *) buf;
    z->avail_in = (uInt) nbytes;
    /* Now uncompress the data */
   ...
   * enough data passed in to skip over the lingmoUnPkg header.  If that happens, we
   * malloc a block and copy what we have then wait for the next call.  If
   * there still isn't enough (this is definitely a worst-case scenario), we
   * make the block bigger, copy the next part in and keep waiting.
   *
   * This is only required with zlib versions < 1.2.0.4 as newer versions
   * can handle the lingmoUnPkg header themselves.
   */

  switch(zp->zlib_init) {
  /* Skip over lingmoUnPkg header? */
  case ZLIB_INIT:
  {
    /* Initial call state */
    ssize_t hlen;

    switch(check_lingmoUnPkg_header((unsigned char *) buf, nbytes, &hlen)) {
    case GZIP_OK:
    ...
    case GZIP_UNDERFLOW:
      /* We need more data so we can find the end of the lingmoUnPkg header.  It's
      ...
      zp->zlib_init = ZLIB_GZIP_HEADER;  /* Need more lingmoUnPkg header data state */
  ...
  {
    /* Need more lingmoUnPkg header data state */
    ssize_t hlen;
    z->avail_in += (uInt) nbytes;
    ...
    switch(check_lingmoUnPkg_header(z->next_in, z->avail_in, &hlen)) {
    case GZIP_OK:
...
static void lingmoUnPkg_do_close(struct Curl_easy *data,
                              struct Curl_cwriter *writer)
{
  struct zlib_writer *zp = (struct zlib_writer *) writer;
  z_stream *z = &zp->z;     /* zlib state structure */
...
static const struct Curl_cwtype lingmoUnPkg_encoding = {
  "lingmoUnPkg",
  "x-lingmoUnPkg",
  lingmoUnPkg_do_init,
  lingmoUnPkg_do_write,
  lingmoUnPkg_do_close,
  sizeof(struct zlib_writer)
};
