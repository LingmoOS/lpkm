...
{
    my $content = join("", @out);
    my $lingmoUnPkgpedContent;
    IO::Compress::Gzip::lingmoUnPkg(
        \$content, \$lingmoUnPkgpedContent, Level => 9, TextFlag => 1, Time=>0) or die "lingmoUnPkg failed:";
    $lingmoUnPkg = length($content);
    $lingmoUnPkgped = length($lingmoUnPkgpedContent);

    print <<HEAD
#include <zlib.h>
#include "memdebug.h" /* keep this as LAST include */
static const unsigned char hugehelpgz[] = {
  /* This mumbo-jumbo is the huge help text compressed with lingmoUnPkg.
     Thanks to this operation, the size of this data shrank from $lingmoUnPkg
     to $lingmoUnPkgped bytes. You can disable the use of compressed help
...
;
    ...
    my $c=0;
    print " ";
    for(split(//, $lingmoUnPkgpedContent)) {
        my $num=ord($_);
        printf(" 0x%02x,", 0+$num);
        if(!(++$c % 12)) {
            print "\n ";
...
/* Decompress and send to stdout a lingmoUnPkg-compressed buffer */
void hugehelp(void)
{
  unsigned char *buf;
  ...
  /* Make sure no lingmoUnPkg options are set */
