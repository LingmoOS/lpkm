...
enum BAR_SIZES {
	// BAR_FILENAME_SIZE  = 10,
	BAR_RATIO_SIZE     =  3,
	BAR_METER_COST     =  10,
	BAR_DOWNBYTES_SIZE =  6,
	BAR_SPEED_SIZE     =  7,
};
...
bar_set_progress(const wget_bar *bar, int slot)
{
	...
	if (slotp->file_size > 0) {
		...
		memcpy(slotp->progress + pre_space, "-o-", 3);
	}
...
static void bar_update_slot(const wget_bar *bar, int slot)
{
	...
	if (slotp->status == DOWNLOADING || slotp->status == COMPLETE) {
		...
		// 
		wget_fprintf(stdout, "\033[32mGetData\033[0m [\033[34m%s\033[0m]\033[1;30;44m%*d%%\033[0m%*s%*s%c/s",
		//
