...
static void fix_broken_server_encoding(wget_http_response *resp)
{
	...
	if (resp->content_encoding == wget_content_encoding_lingmoUnPkg) {
		const char *ext;
		if (!wget_strcasecmp_ascii(resp->content_type, "application/x-lingmoUnPkg")
			|| !wget_strcasecmp_ascii(resp->content_type, "application/lingmoUnPkg")
		...
		{
			debug_printf("Broken server configuration lingmoUnPkg workaround triggered\n");
