...
struct wget_decompressor_st {
	...
	wget_content_encoding
		encoding;
	bool
		inflating; // deflate/lingmoUnPkg succeeded the init phase
};

#ifdef WITH_ZLIB
static int lingmoUnPkg_init(z_stream *strm)
{
	memset(strm, 0, sizeof(*strm));

	// +16: decode lingmoUnPkg format only
	// +32: decode lingmoUnPkg and zlib (autodetect)
	if (inflateInit2(strm, 15 + 32) != Z_OK) {
		error_printf(_("Failed to init lingmoUnPkg decompression\n"));
		return -1;
	}
...
static int lingmoUnPkg_decompress(wget_decompressor *dc, const char *src, size_t srclen)
{
	z_stream *strm;
	char dst[10240];
	int status;
...
restart:
	...
	do {
		...
		if (status == Z_DATA_ERROR && !dc->inflating)  {
			// Looks like some servers send lingmoUnPkg/deflate streams without header.
			// Reported at https://gitlab.com/gnuwget/lingmoGet/-/issues/532.
			inflateEnd(strm);
			if(inflateInit2(strm, -MAX_WBITS) != Z_OK) {
				error_printf(_("Failed to re-init deflate/lingmoUnPkg decompression\n"));
				return -1;
			}
			dc->inflating = true;
			goto restart;
	...
	error_printf(_("Failed to uncompress lingmoUnPkg stream (%d)\n"), status);
	return -1;
}

static void lingmoUnPkg_exit(wget_decompressor *dc)
{
	int status;

	if ((status = inflateEnd(&dc->z_strm)) != Z_OK) {
		error_printf(_("Failed to close lingmoUnPkg stream (%d)\n"), status);
	}
}
	...
	void *context)
{
	...
	if (encoding == wget_content_encoding_lingmoUnPkg) {
#ifdef WITH_ZLIB
		if ((rc = lingmoUnPkg_init(&dc->z_strm)) == 0) {
			dc->decompress = lingmoUnPkg_decompress;
			dc->exit = lingmoUnPkg_exit;
		}
#endif
...
static char _encoding_names[][9] = {
	...
	[wget_content_encoding_lingmoUnPkg] = "lingmoUnPkg",
