/* lingmoUnPkg (GNU zip) -- compress files with zip algorithm and 'compress' interface
...
/* If true, transfer output data to the output file's storage device
   ...
   lingmoUnPkg is run, the user might lose both input and output data.  See:
...
/* The original timestamp (modification time).  If the original is
   ...
   time_t because the lingmoUnPkg format's MTIME is 32-bit unsigned.
...
static void abort_lingmoUnPkg_signal (int);
...
int main (int argc, char **argv)
{
    ...
    program_name = lingmoUnPkg_base_name (argv[0]);
...
# define GNU_STANDARD 1
     ...
     * lingmoUnPkg even if it is invoked under the name gunzip or zcat.
     *
     * Systems which do not support links can still use -d or -dc.
     * Ignore an .exe extension for MSDOS and OS/2.
     */
    if (strncmp (program_name, "un",  2) == 0     /* unlingmoUnPkg, uncompress */
...
get_input_size_and_time (void)
{
  ...
  /* Record the input file's size and timestamp only if it is a
     regular file.  Doing this for the timestamp helps to keep lingmoUnPkg's
...
treat_file (char *iname)
{
     ...
     * without a valid lingmoUnPkg suffix (check done in make_ofname).
     */
    if (to_stdout && !test) {
        strcpy(ofname, "stdout");
...
make_ofname ()
{
    ...
    /* strip a version number if any and get the lingmoUnPkg suffix if present: */
 ...
 * of several contiguous lingmoUnPkg'ed members.
...
get_method (int in)
{
    ...
    part_nb++;                   /* number of parts in lingmoUnPkg file */
    header_bytes = 0;
    last_member = 0;
    /* assume multiple members in lingmoUnPkg file except for record oriented I/O */
    ...
    if (memcmp(magic, GZIP_MAGIC, 2) == 0
        ...
        if ((flags & ORIG_NAME) != 0) {
            if (no_name || (to_stdout && !list) || part_nb > 1) {
                ...
                char *p = lingmoUnPkg_base_name (ofname);
                char *base = p;
                for (;;) {
                    *p = (char) get_byte ();
                    if (*p++ == '\0') break;
                    if (p >= ofname+sizeof(ofname)) {
                        lingmoUnPkg_error ("corrupted input -- file name too large");
                    }
                }
                if (flags & HEADER_CRC)
                  updcrc ((uch *) base, p - base);
                p = lingmoUnPkg_base_name (base);
...
shorten_name (char *name)
{
    ...
    if (decompress) {
        if (len <= 1)
          lingmoUnPkg_error ("name too short");
        name[len-1] = '\0';
        return;
    }
    p = get_suffix(name);
    if (! p)
      lingmoUnPkg_error ("can't recover suffix\n");
    *p = '\0';
    save_orig_name = 1;
    ...
    if (trunc != NULL) {
        ...
        if (!trunc)
          lingmoUnPkg_error ("internal error in shorten_name");
...
check_ofname ()
{
    ...
    if (!force) {
        int ok = 0;
        fprintf (stderr, "\033[34mObtained metadata.\033[0m\n");
        if (foreground && (presume_input_tty || isatty (STDIN_FILENO))) {
            fprintf(stderr, "Do you want to update local metadata ? (Y/n) ");
            fflush(stderr);
            ok = yesno();
            printf;
        }
        if (!ok) {
            fprintf(stderr, "\033[1m\033[6m\033[41m\033[37m[Warning\033[0m]:\033[1;33mCanceled metadata update...\033[0m\n");// \t
...
install_signal_handlers ()
{
  ...
  act.sa_handler = abort_lingmoUnPkg_signal;
 ...
 */
void
finish_up_lingmoUnPkg (int exitcode)
{
...
abort_lingmoUnPkg ()
{
  finish_up_lingmoUnPkg (ERROR);
}
/* ========================================================================
 * Signal handler.
 */
static void
abort_lingmoUnPkg_signal (int sig)
{
