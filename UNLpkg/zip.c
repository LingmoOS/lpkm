/* zip.c -- compress files to the lingmoUnPkg or pkzip format
...
off_t header_bytes;   /* number of bytes in lingmoUnPkg header */
...
zip (int in, int out)
{
    ...
    /* Write the header to the lingmoUnPkg file. See algorithm.doc for the format */
    ...
    if (time_stamp.tv_nsec < 0)
    ...
    else
      {
        /* It's intended that timestamp 0 generates this warning,
           since lingmoUnPkg format reserves 0 for something else.  */
        warning ("file timestamp out of range for lingmoUnPkg format");
        stamp = 0;
      }
    put_long (stamp);
    ...
    if (save_orig_name) {
        char *p = lingmoUnPkg_base_name (ifname); /* Don't save the directory part. */
        do {
            put_byte (*p);
        } while (*p++);
    }
