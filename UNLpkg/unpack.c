...
read_byte (void)
{
  int b = get_byte ();
  if (b < 0)
    lingmoUnPkg_error ("invalid compressed data -- unexpected end of file");
  return b;
}
...
read_tree ()
{
    ...
    if (! (0 < max_len && max_len <= MAX_BITLEN))
      lingmoUnPkg_error ("invalid compressed data -- "
    ...
    for (len = 1; len <= max_len; len++) {
        ...
        if (max_leaves - (len == max_len) < leaves[len])
          lingmoUnPkg_error ("too many leaves in Huffman tree");
...
build_tree ()
{
    ...
    if ((nodes >> 1) != 1)
      lingmoUnPkg_error ("too few leaves in Huffman tree");
...
unpack (int in, int out)
{
    ...
    if (orig_len != (ulg)(bytes_out & 0xffffffff)) {
        lingmoUnPkg_error ("invalid compressed data--length error");
    }
    return OK;
}
