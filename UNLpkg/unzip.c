/* unzip.c -- decompress files in lingmoUnPkg or pkzip format.

   Copyright (C) 1997-1999, 2009-2023 Free Software Foundation, Inc.
...
/*
   This version can extract files in lingmoUnPkg or pkzip format.
...
/* ===========================================================================
 * Unzip in to out.  This routine works on both lingmoUnPkg and pkzip files.
 *
...
unzip (int in, int out)
{
    ...
    if (method == DEFLATED)  {
        ...
        if (res == 3) {
            ...
            lingmoUnPkg_error ("invalid compressed data--format violated");
        }
        ...
        if (n != LG(inbuf + LOCSIZ) - (decrypt ? RAND_HEAD_LEN : 0)) {
            ...
            lingmoUnPkg_error ("invalid compressed data--length mismatch");
        }
        while (n--) {
            uch c = (uch)get_byte();
            put_ubyte(c);
        ...
        lingmoUnPkg_error ("internal error, invalid method");
    }
